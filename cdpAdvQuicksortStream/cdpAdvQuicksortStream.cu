                           ...
                           unsigned int depth)
{
  ...
  {
    if (lt_count > 0)
      lt_offset = atomicAdd((unsigned int *) &(atomicData+streamNo)->lt_offset, lt_count);

    if (gt_count > 0)
      gt_offset = len - (atomicAdd((unsigned int *) &(atomicData+streamNo)->gt_offset, gt_count) + gt_count);
  }
  ...
  if (lane_id == 0)
  {
    ...
    if (atomicAdd((unsigned int *) &(atomicData+streamNo)->sorted_count, mycount) + mycount == len)
    {
      // We're the last warp to do any sorting. Therefore it's up to us to launch the next stage.
      unsigned int lt_len = (atomicData + streamNo)->lt_offset;
      unsigned int gt_len = (atomicData + streamNo)->gt_offset;
      ...
      ringbufFree<qsortAtomicData>(atomicDataStack + streamNo, atomicData + streamNo);
      ...
      if (lt_len > BITONICSORT_LEN)
      {
        ...
        else
        {
          ...
          if ((&(atomicData+streamNo) = ringbufAlloc<qsortAtomicData>(&(atomicDataStack + streamNo))) == NULL)
            printf("Stack-allocation error. Failing left child launch.\n");
          else
          {
            (atomicData + streamNo)->lt_offset = (atomicData+streamNo)->gt_offset = (atomicData+streamNo)->sorted_count = 0;
            unsigned int numblocks = (unsigned int)(lt_len+(QSORT_BLOCKSIZE-1)) >> QSORT_BLOCKSIZE_SHIFT;
            qsort_warp<<< numblocks, QSORT_BLOCKSIZE, 0, lstream >>>(outdata, indata, offset, lt_len, atomicData, atomicDataStack, streamNo, !source_is_indata, depth+1);
          }
        }
      }
      else if (lt_len > 1)
      ...
      if (gt_len > BITONICSORT_LEN)
      {
        ...
        else
        {
          ...
          else
          {
            ...
            qsort_warp<<< numblocks, QSORT_BLOCKSIZE, 0, rstream >>>(outdata, indata, offset+lt_len, gt_len, atomicData, atomicDataStack, streamNo, !source_is_indata, depth+1);
          }
        }
      }
      else if (gt_len > 1)
...
*/
void run_quicksort_cdp(unsigned *gpudata, 
                        unsigned *scratchdata, 
                        unsigned int count, 
                        ...
                        cudaStream_t stream)
{
  ...
  checkCudaErrors(cudaMemcpyAsync(ringbuf, &buf, sizeof(buf), cudaMemcpyHostToDevice, stream));
  ...
  if (count > BITONICSORT_LEN)
  {
    ...
    qsort_warp<<< numblocks, QSORT_BLOCKSIZE, 0, stream >>>(gpudata, scratchdata, soffset, count, gpustack, ringbuf, true, 0);
  }
  else
  {
    bitonicsort<<< 1, BITONICSORT_LEN, 0, stream >>>(gpudata, gpudata, 0, count);
  }
...
int run_qsort(unsigned int size, int seed, int debug, int loop, int verbose)
{
  ...
  unsigned int nStreams = 32;
  unsigned int streamSize = size / nStreams;
  ...
  // Create gpu stack for nStreams
  unsigned int stacksize = QSORT_STACK_ELEMS;
  qsortAtomicData *gpustack;
  checkCudaErrors(cudaMalloc((void **)&gpustack, nStreams * stacksize * sizeof(qsortAtomicData)));
  checkCudaErrors(cudaMemset(gpustack, 0, nStreams * stacksize * sizeof(qsortAtomicData)));

  // Create ringbuffer (cpu and gpu) for nStreams
  qsortRingbuf *buf, *ringbuf;
  checkCudaErrors(cudaMallocHost((void **)&buf, nStreams * sizeof(qsortRingbuf))); // host pinned
  checkCudaErrors(cudaMalloc((void **)&ringbuf, nStreams * sizeof(qsortRingbuf)));

  // Create CPU data.
  unsigned *data;
  checkCudaErrors(cudaMallocHost((void **)&data, size*sizeof(unsigned))); // host pinned
  ...
  for (size=min; size<=max; size+=loop)
  {
    ...
    for (unsigned int j=0; j < nStreams; ++j)
      checkCudaErrors( cudaStreamCreate(&stream[j]) );
    ...
    for (unsigned int j=0; j < nStreams; ++j)
    { 
      // async copy the data for this stream
      unsigned int soffset = j * streamSize;
      checkCudaErrors(cudaMemcpyAsync(&gpudata[soffset], &data[soffset], 
            streamSize*sizeof(unsigned), cudaMemcpyHostToDevice,
            stream[j]));

      // async copy ringbuf
      &(buf + j)->head = 1;  // We start with one allocation
      &(buf + j)->tail = 0;
      &(buf + j)->count = 0;
      &(buf + j)->max = 0;
      &(buf + j)->stacksize = stacksize;
      &(buf + j)->stackbase = gpustack[j];
      checkCudaErrors(cudaMemcpyAsync(&(ringbuf + j), &(buf + j),
            sizeof(qsortRingbuf), cudaMemcpyHostToDevice,
            stream[j]));

      // launch kernel
      unsigned int numblocks = (unsigned int)(count+(QSORT_BLOCKSIZE-1)) >> QSORT_BLOCKSIZE_SHIFT;
      qsort_warp<<< numblocks, QSORT_BLOCKSIZE, 0, stream >>>(gpudata, scratchdata, soffset, count, gpustack, ringbuf, j, true, 0);

      // So we're now populated and ready to go! We size our launch as
      // blocks of up to BLOCKSIZE threads, and appropriate grid size.
      //run_quicksort_cdp(gpudata, scratchdata, streamSize, soffset, stream[j]);

      // async copy data back
      checkCudaErrors(cudaMemcpyAsync(&data[soffset], &gpudata[soffset], 
            streamSize*sizeof(unsigned), cudaMemcpyDeviceToHost,
            stream[j]));
    }
    ...
    for (unsigned int j=0; j < nStreams; ++j)
    {
      ...
      for (check=1; check<streamSize; check++)
      {
        if (data[soffset+check] < data[soffset+check-1])
        {
    ...
    printf("Sorted %u groups of %u elems in %.3f ms (%.3f Melems/sec)\n", nStreams, streamSize, elapse, (float)size/(elapse*1000.0f));
    fflush(stdout);
  }
