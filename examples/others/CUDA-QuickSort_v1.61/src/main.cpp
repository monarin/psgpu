...
void compareTest(string& select,int device)
{
	...
	double timerQuick,timerQuick2,timerBitonic,timerMerge,timerThrust;
	...
	while( i_dist < 6 )
	{
		...
		while(dataSize<=N)
		{
			...
			for(int i=0;i<NUMBER_DATASET;i++)
			{
				...
				test_thrustSort(inData,dataSize,&timerThrust);
